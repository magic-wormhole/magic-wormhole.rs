...
impl Code {
    ...
    fn in_idle(&mut self, event: CodeEvent) -> (Option<State>, Events) {
        ...
        match event {
            AllocateCode(length, wordlist) => {
                (Some(State::Allocating), events![A_Allocate(length, wordlist)])
            }
            InputCode => (Some(State::InputtingNameplate),
                          events![I_Start]), // TODO: return Input object
            SetCode(code) => {
                ...
                let code = nc[1];
                (Some(State::Known),
                 events![N_SetNameplate(nameplate.to_string()),
    ...
    fn in_inputting_nameplate(&mut self, event: CodeEvent) -> (Option<State>, Events) {
        use events::CodeEvent::*;
        match event {
            ...
            SetCode(code) => panic!(),
            Allocated(nameplate, code) => panic!(),
            GotNameplate(nameplate) => (Some(State::InputtingWords),
                                        events![N_SetNameplate(nameplate)]),
            FinishedInput(_code) => panic!(),
        }
    }

    fn in_inputting_words(&mut self, event: CodeEvent) -> (Option<State>, Events) {
        use events::CodeEvent::*;
        match event {
            AllocateCode(_length, _wordlist) => panic!(),
            InputCode => panic!(),
            ...
            Allocated(nameplate, code) => panic!(),
            GotNameplate(nameplate) => panic!(),
            FinishedInput(code) => (Some(State::Known),
                                    events![B_GotCode(code.to_string()),
                                            K_GotCode(code.to_string())]),
        }
    }
